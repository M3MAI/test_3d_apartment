...
function applyRoomOverrides() {
  ...
  ROOMS.forEach(r => {
    ...
    if (o.wallColors) r.wallColors = o.wallColors;
...
function renderRoomShell(room) {
  ...
  // Per-wall colored strips — draw a colored stripe on each wall edge to
  // show the individual wall colors in 2D (matches the per-wall color pickers).
  const stripe = 10;
  const getWC = (wId) => {
    if (typeof resolveWallColor === 'function') return resolveWallColor(room, wId);
    if (room.accentColor && room.accentWall === wId) return room.accentColor;
    return room.wallColor || '#cccccc';
  };
  parts.push(`<rect x="${px}" y="${py}" width="${w}" height="${stripe}" fill="${safeColor(getWC('top'), '#ccc')}" opacity="0.85" rx="2" />`);
...
// Helper: effective dimensions accounting for instance-level overrides.
function effectiveDims(item, inst) {
  return {
    w: inst.overrideW || item.w,
    h: inst.overrideH || item.h,
    depth: inst.overrideDepth || item.depth || 80,
  };
}
...
function detectCollisions(items) {
  ...
  const boxes = items.map(inst => {
    ...
    if (!item) return null;
    const dims = effectiveDims(item, inst);
    return obb(inst.x, inst.y, dims.w, dims.h, inst.rotation || 0, inst.instId);
  }).filter(Boolean);
...
function renderSelection() {
  ...
  const isCustom = inst.groupId === "custom" && item.image;
  // Image thumbnail for custom items
  const thumbHtml = isCustom
    ? `<img class="sel-thumb" src="${esc(item.image)}" alt="${esc(item.name)}" />`
    : "";
  panel.innerHTML = `
    ${thumbHtml}
    <div class="sel-row"><span>الاسم</span><b>${esc(item.icon || "📦")} ${esc(item.name)}</b></div>
    <div class="sel-row"><span>الأبعاد</span><b>${fmtPair(dims.w, dims.h)}</b></div>
    <div class="sel-row"><span>الموقع</span><b>X: ${fmtCm(Number(inst.x) || 0)} , Y: ${fmtCm(Number(inst.y) || 0)}</b></div>
    <div class="dim-section">
      <div class="dim-section-title">تغيير الأبعاد</div>
      <div class="dim-control">
        <span class="dim-label">W</span>
        <input type="range" class="dim-slider" data-dim="w" min="10" max="500" step="5" value="${dims.w}" />
        <input type="number" class="dim-num" data-dim="w" min="10" max="500" step="5" value="${dims.w}" /> <small>سم</small>
      </div>
      <div class="dim-control">
        <span class="dim-label">D</span>
        <input type="range" class="dim-slider" data-dim="h" min="10" max="500" step="5" value="${dims.h}" />
        <input type="number" class="dim-num" data-dim="h" min="10" max="500" step="5" value="${dims.h}" /> <small>سم</small>
      </div>
      <div class="dim-control">
        <span class="dim-label">H</span>
        <input type="range" class="dim-slider" data-dim="depth" min="5" max="350" step="5" value="${dims.depth}" />
        <input type="number" class="dim-num" data-dim="depth" min="5" max="350" step="5" value="${dims.depth}" /> <small>سم</small>
      </div>
    </div>
    <div class="sel-row">
      <span>الدوران</span>
  ...
  // Dimension slider bindings
  let dimPushed = false;
  panel.querySelectorAll(".dim-slider, .dim-num").forEach(input => {
    const dim = input.dataset.dim; // "w", "h", or "depth"
    input.addEventListener("input", () => {
      if (!dimPushed) { pushHistory(); dimPushed = true; }
      const v = Math.max(5, Math.min(500, parseInt(input.value, 10) || 10));
      // Sync slider <-> number input
      panel.querySelectorAll(`[data-dim="${dim}"]`).forEach(el => { if (el !== input) el.value = v; });
      // Set instance override
      if (dim === "w")     inst.overrideW     = v;
      if (dim === "h")     inst.overrideH     = v;
      if (dim === "depth") inst.overrideDepth  = v;
      fitWithinRoom(inst);
      saveLayouts();
      drawRoom();
    });
    input.addEventListener("change", () => { dimPushed = false; });
  });
...
function fitWithinRoom(inst) {
  ...
  const effW = rot === 90 ? dims.h : dims.w;
  const effH = rot === 90 ? dims.w : dims.h;
  inst.x = clamp(inst.x, effW/2, room.width  - effW/2);
  inst.y = clamp(inst.y, effH/2, room.depth  - effH/2);
}
...
function bindRoomModal() {
  ...
  if (applyAllBtn) {
    applyAllBtn.addEventListener("click", () => {
      const topColor = document.getElementById("re-wall-top").value;
      ["bottom", "left", "right"].forEach(wId => {
        document.getElementById("re-wall-" + wId).value = topColor;
      });
    });
  }
  ...
  openBtn.addEventListener("click", () => {
    ...
    // Load per-wall colors
    const wc = room.wallColors || {};
    const fallback = room.wallColor || "#cccccc";
    document.getElementById("re-wall-top").value    = wc.top    || (room.accentWall === "top"    && room.accentColor ? room.accentColor : fallback);
    document.getElementById("re-wall-bottom").value = wc.bottom || (room.accentWall === "bottom" && room.accentColor ? room.accentColor : fallback);
    document.getElementById("re-wall-left").value   = wc.left   || (room.accentWall === "left"   && room.accentColor ? room.accentColor : fallback);
  ...
  saveBtn.addEventListener("click", () => {
    ...
    const wallColors = {
      top:    document.getElementById("re-wall-top").value,
      bottom: document.getElementById("re-wall-bottom").value,
      left:   document.getElementById("re-wall-left").value,
      right:  document.getElementById("re-wall-right").value,
    };
    // Use the north wall as the primary wallColor (for backward compat / room swatch)
    const wallColor = wallColors.top;
